...
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     蜂鸣器更新函数（需要定时调用，建议1ms周期）
// 参数说明     void
// 返回参数     void
// 使用示例     buzzer_update(); // 在1ms定时器中断中调用
// 备注信息     负责处理蜂鸣器的自动鸣叫逻辑
//-------------------------------------------------------------------------------------------------------------------
void buzzer_update(void)
{
    buzzer_time_ms++; // 更新内部时间
...
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     设置系统时间（毫秒）
// 参数说明     time_ms: 系统时间（毫秒）
// 返回参数     void
// 使用示例     buzzer_set_time(system_time_ms);
// 备注信息     用于同步蜂鸣器的时间基准，如果有全局时间可以调用此函数同步
//-------------------------------------------------------------------------------------------------------------------
void buzzer_set_time(uint32 time_ms)
{
    buzzer_time_ms = time_ms;
}
