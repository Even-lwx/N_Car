...
/**
 * @brief 遍历所有页面并收集参数数据
 * @param data_buffer 输出缓冲区（uint32数组）
 * @return 参数总数
 */
static uint32 Collect_All_Params(uint32 *data_buffer)
{
            ...
            for(uint8 i = 0; i < page->len; i++)
            {
                ...
                switch(param->type)
                {
                    case data_float_show:
                    {
                        ...
                        // 将float按位复制到uint32
                        data_buffer[param_index] = *(uint32*)&value;
                        break;
                    }
 ...
 * @return 1=成功, 0=失败
 */
static uint8 Restore_All_Params(uint32 *data_buffer, uint32 param_count)
{
    uint32 param_index = 0;
        ...
        if(page->data != NULL && page->len > 0)
        {
            ...
            for(uint8 i = 0; i < page->len; i++)
            {
                ...
                switch(param->type)
                {
                    case data_float_show:
                    {
                        uint32 raw_data = data_buffer[param_index];
                        // 将uint32按位复制回float
                    ...
                    case data_int16_show:
                    {
                        *(int16*)param->address = (int16)data_buffer[param_index];
                        break;
                    }
                    
                    case data_int_show:
                    {
                        *(int*)param->address = (int)data_buffer[param_index];
                        break;
                    }
                    
                    case data_uint32_show:
                    {
                        *(uint32*)param->address = data_buffer[param_index];
                        break;
                    }
                }
...
uint8 Param_Save_All(void)
{
    ...
    // 1. 自动收集所有参数
    save_data.param_count = Collect_All_Params(save_data.data);
...
uint8 Param_Load_All(void)
{
    ...
    // 4. 自动恢复所有参数
    Restore_All_Params(load_data.data, load_data.param_count);
