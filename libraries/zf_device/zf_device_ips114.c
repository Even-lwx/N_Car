...
void ips114_show_string (uint16 x, uint16 y, const char dat[])
{
    ...
    uint16 j = 0;
    /*
     * 计算当前字体宽度（像素），在绘制前检查是否会超出屏幕宽度。
     * 这样可以在字符串长度过长时安全地截断，避免 ips114_set_region 的断言触发。
     */
    uint16 font_w = 6;
    switch(ips114_display_font)
    {
        case IPS114_6X8_FONT:   font_w = 6; break;
        case IPS114_8X16_FONT:  font_w = 8; break;
        case IPS114_16X16_FONT: font_w = 16; break;    // 预设值，若未来支持可用
    }

    while('\0' != dat[j])
    {
        uint16 char_x = x + font_w * j;
        /* 如果下一个字符的最右像素会超出屏幕，则停止绘制 */
        if ((uint32)char_x + (uint32)font_w - 1u >= (uint32)ips114_width_max)
        {
            break;
        }
        ips114_show_char(char_x, y, dat[j]);
        j ++;
    }
}
